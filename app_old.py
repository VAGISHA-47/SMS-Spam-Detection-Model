...
with left:
    ...
    input_sms = st.text_area('Enter SMS to classify', height=120)
    if st.button('Predict'):
        if not input_sms or input_sms.strip() == '':
            ...
            with st.expander('Show decoding steps'):
                ...
                st.write('**Alpha tokens**')
                st.write(steps['tokens_alpha'])
                st.write('**After stopwords**')
                st.write(steps['after_stop'])
                st.write('**After lemmatize**')
                st.write(steps['after_lemmatize'])
                st.write('**After stem**')
                st.write(steps['after_stem'])
...
st.markdown("*Made by Vagisha Yadav*")
...
input_sms = st.text_input("Enter SMS to classify")
if st.button('Predict'):
    if not input_sms or input_sms.strip() == "":
        st.warning("Please enter an SMS message to classify.")
    else:
...
if st.session_state.get('logged_in') and st.session_state.get('user_email'):
    try:
        ...
        if not items:
            ...
            for it in items:
                sms_text = it.get('text') or ''
                label = it.get('label', 'n/a').upper()
                ts = it.get('timestamp', '')
                if sms_text:
                    st.write(f"**{label}** — {sms_text} — {ts}")
                else:
                    st.write(f"**{label}** — [No SMS text found] — {ts}")
    except Exception as e:
